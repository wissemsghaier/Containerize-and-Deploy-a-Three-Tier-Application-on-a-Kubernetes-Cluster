...
//         //let result = await fetch("http://backend-service:5000/add-product", 
//         //const backendUrl = process.env.REACT_APP_BACKEND_URL;
//         //let result = await fetch(`${backendUrl}/add-product`, 
...
const AddProduct = () => {
    ...
    const navigate = useNavigate();
    const backendUrl = process.env.REACT_APP_BACKEND_URL; //URL backend provenant du ConfigMap
    let showError = false;
    ...
    return (
        <div className="bodyContainer">
            ...
            />
            {error && !category && <p className="error-text">Please Enter Product Category</p>}
            <button type="button" className="btn" onClick={addProductData}>
                Add Product
            </button>
